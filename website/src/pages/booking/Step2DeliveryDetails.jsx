...
const SAVED_ADDRESS = {
  label: 'Home',
  isDefault: true,
  line1: 'House No. 12-5-678, Road No. 3, Banjara Hills',
  city: 'Hyderabad',
  state: 'Telangana',
  pincode: '500034',
};
...
export default function Step2DeliveryDetails({ order, updateOrder, onNext, onPrevious, onEditStep1 }) {
  const [mode, setMode] = useState('saved');
  const [newAddress, setNewAddress] = useState({ house: '', street: '', landmark: '', city: '', state: '', pincode: '' });
  const [contact, setContact] = useState({ name: 'Rahul Kumar', mobile: '+91 91234 56789', altMobile: '' });
  ...
  const handleNext = () => {
    const address =
      mode === 'saved'
        ? { line1: SAVED_ADDRESS.line1, city: SAVED_ADDRESS.city, state: SAVED_ADDRESS.state, pincode: SAVED_ADDRESS.pincode }
        : {
    ...
    <div className="flex flex-col lg:flex-row gap-8 items-start">
      ...
      <section className="mb-8">
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-4">
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Deliver to a saved address</p>
            </div>
          </button>
          <button
            type="button"
        ...
        <button
          type="button"
          onClick={() => setMode('saved')}
          className={`w-full flex items-start gap-3 border rounded-xl p-4 text-left transition-colors mb-6 ${
            mode === 'saved' ? 'border-brand-green bg-brand-green/5' : 'border-[#EDE2CD]'
          }`}
        >
          <span
            className={`w-4 h-4 mt-0.5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
              mode === 'saved' ? 'border-brand-green' : 'border-gray-300'
            }`}
          >
            {mode === 'saved' && <span className="w-2 h-2 rounded-full bg-brand-green" />}
          </span>
          <div className="flex-1">
            <div className="flex items-center gap-2">
              <p className="text-sm font-bold text-gray-900">{SAVED_ADDRESS.label}</p>
              {SAVED_ADDRESS.isDefault && (
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {SAVED_ADDRESS.line1}, {SAVED_ADDRESS.city}, {SAVED_ADDRESS.state} - {SAVED_ADDRESS.pincode}
            </p>
          </div>
        ...
        <div onFocus={() => setMode('new')} onClick={() => setMode('new')}>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            <div>
              <Label>State</Label>
              <select value={newAddress.state} onChange={(e) => setField('state', e.target.value)} className={inputClass}>
      ...
      <section className="mb-8">
        ...
        <div className="relative w-full h-56 rounded-xl overflow-hidden border border-[#EDE2CD] bg-[#E9ECE3]">
          ...
          />
          <div className="absolute top-6 right-10 w-24 h-16 rounded-full bg-[#CFE3CE] opacity-70 blur-sm" />
          <MapPin
            size={36}
        ...
        <div className="mt-3 flex items-center justify-between gap-2 bg-[#EEF3E4] border border-[#DCE7C8] rounded-lg px-4 py-3">
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Road No. 3, Banjara Hills, Hyderabad, Telangana - 500034</p>
            </div>
          </div>
          <button type="button" className="text-xs font-semibold text-gray-500 hover:text-gray-700 flex-shrink-0">
            Clear
          </button>
        </div>
      </section>
