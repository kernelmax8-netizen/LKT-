...
import { fetchWoodTypes } from '../../services/catalogService'
...
const iconByKey = Object.fromEntries(woodTypeOptions.map((w) => [w.key, w.icon]))
...
export default function Step1Requirements({ order, updateOrder, onNext }) {
  const [options, setOptions] = useState(woodTypeOptions)

  useEffect(() => {
    fetchWoodTypes().then((list) => {
      setOptions(
        list.map((w) => ({
          key: w.key,
          label: w.label,
          icon: iconByKey[w.key] || woodTypeOptions[0].icon,
        })),
      )
    })
  }, [])
  ...
  const toggleWood = (opt) => {
    ...
    const next = exists
      ? order.woodTypes.filter((w) => w.key !== opt.key)
      : [...order.woodTypes, { key: opt.key, label: opt.label }]
    updateOrder({ woodTypes: next })
  }

  const canProceed =
    order.woodTypes.length > 0 &&
    order.quantity &&
    order.purpose &&
    ...
    <div className="flex flex-col lg:flex-row gap-8 items-start">
      <div className="flex-1 min-w-0 bg-white border border-[#F1E7D4] rounded-2xl p-6 sm:p-8">
        ...
        <section className="mb-8">
            ...
            {deliveryPrefs.map((d) => {
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    updateOrder({
                      deliveryPreference: d.key,
                      deliveryDate: d.key === 'Select Date' ? order.deliveryDate : '',
                    })
                  }
          ...
          {order.deliveryPreference === 'Select Date' && (
            <div className="mt-4 sm:w-1/2">
              <label className="block text-xs font-bold text-gray-800 mb-1.5">Delivery Date</label>
              <input
                type="date"
